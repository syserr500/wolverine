...
/// <summary>
/// Awaited typed replies through automatic global partition routing. A
/// <see cref="LocalPartitionedMessageTopology"/> stands in as the "external" topology so the partitioning
/// shape is exercised in process with no broker, as <see cref="sticky_handlers_with_global_partitioning"/>
/// does. The broker hop and cross-node partition ownership are covered by the two-node test in SlowTests.
/// </summary>
public class awaited_replies_through_global_partitioning
{
    ...
    public async Task shard_selection_matches_the_publishing_path()
    {
        ...
        // If the two paths ever hash differently, awaiting a reply silently breaks the per-group ordering
        // that publishing guarantees
    ...
    {
        // Asserting on the send-time header rather than on Envelope.GroupId in the handler: the receiving
        // side re-derives a group id of its own when it shards into a lane, so the handler would see one
        // even if nothing had been stamped on the wire.
        using var host = await startHostAsync();
        var bus = host.MessageBus();
        ...
        // ...and it matches what the publishing path stamps for the same message
    ...
    public async Task explicit_delivery_options_group_id_wins_over_the_grouping_rule()
    {
        ...
        // ...and the override decides the shard too, not just the metadata
    ...
    {
        // Nothing handles NobodyHandlesThis, so the partition slot answers with a FailureAcknowledgement
        // from NoHandlerContinuation. Before GlobalPartitionedRoute implemented IMessageInvoker this threw
        // InvalidCastException out of WolverineRuntime.findInvoker instead.
        using var host = await startHostAsync();
        var bus = host.MessageBus();
        ...
        // Plain InvokeAsync<T> too: with no local handler it falls through to the same single route
    ...
    {
        // The partition slot here is a local queue, which MoveToErrorQueue's unsolicited-ack rule excludes
        // by scheme. A solicited reply has to be answered anyway, or the caller only ever sees a timeout.
        using var host = await startHostAsync();
        var bus = host.MessageBus();
    ...
    {
        // The other half: EnableAutomaticFailureAcks still governs the ack back to a plain publisher, and
    ...
    {
        // Asserted against the route directly because a native-ack slot needs a transport that supports
        // EndpointMode.NativeAck to bootstrap at all
        using var host = await startHostAsync();
    ...
    public async Task refuses_direct_reentry_onto_the_lane_the_handler_occupies()
    {
        ...
        // Two DIFFERENT group ids on the same slot and the same lane within it -- the case a guard keyed
        // on group id equality would miss
    ...
    /// <summary>
    /// A group id on the same slot as <paramref name="seed"/> but a different lane within it. The slot has
    /// to match: a different slot is a different queue, which the guard rejects before it ever compares
    /// lanes, so such a pairing would prove nothing about the lane comparison.
    /// </summary>
    ...
    /// <summary>
    /// A sequential lane is the pair (sending slot, processing lane within it). Computed with the
    /// framework's own hashes so the test cannot drift from the implementation.
    /// </summary>
...
/// <summary>
/// Records Envelope.GroupId as an outgoing envelope rule, so a test can tell "resolved before dispatch"
/// apart from "the receiver re-derived one".
/// </summary>
