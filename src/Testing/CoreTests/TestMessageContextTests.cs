...
public class TestMessageContextTests
{
    ...
    public async Task invoke_through_routing_is_recorded_separately_from_inline_invocation()
    {
        ...
        // Both were "invoked", but only one asked to go through routing -- which is what lets a test of an
        // HTTP endpoint assert that the command was partitioned rather than executed in process.
