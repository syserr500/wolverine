...
internal class MoveToErrorQueue : IContinuation
{
        ...
        DateTimeOffset now, Activity? activity)
    {
        ...
        // A caller blocked on a reply or ack it explicitly requested has to be answered on every
        // transport, including the local queue that the unsolicited rule below excludes. Same rule as
        // NoHandlerContinuation and MessageContext.AssertAnyRequiredResponseWasGenerated, which answer a
        // solicited reply unconditionally.
        var wasSolicited = lifecycle.Envelope.AckRequested || lifecycle.Envelope.ReplyRequested.IsNotEmpty();

        // EnableAutomaticFailureAcks governs only the unsolicited ack back to a plain publisher.
